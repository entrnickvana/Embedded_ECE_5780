...
volatile uint8_t Ki = 80;            // Integral gain
...
void pwm_init(void) {
    ...
    /// TODO: Set up a few GPIO output pins for direction control
			GPIOA->MODER |= GPIO_MODER_MODER5_0 | GPIO_MODER_MODER6_0;  // PA5 and PA6 to general output mode
			
	
    /// TODO: Initialize one direction pin to high, the other low
			GPIOA->ODR |= GPIO_ODR_5;			//set PA6 to be hi
	
			GPIOA->ODR &= ~(GPIO_ODR_6);  //Clear PA5 to be low
...
void encoder_init(void) {
     ...
     */
         TIM6->PSC = 8U; // TODO: Change this!
         TIM6->ARR = 14285U; // TODO: // ARR = 14285
...
void PI_update(void) {
    ...
    /// TODO: calculate error signal and write to "error" variable
			...
			// Error = Target_RPM - Actual RPM where Actual_RPM = 
	
			int counts_per_min; 
			counts_per_min = 8400*target_rpm;
			
			error = counts_per_min - motor_speed;			
    ...
    /// TODO: Clamp the value of the integral to a limited positive range
		...
		if(error_integral >= max_error)
			error_integral = max_error;
